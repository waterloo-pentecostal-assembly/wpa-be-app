...
class NewTestimonyForm extends StatefulWidget {
  final OverlayEntry? entry;

  const NewTestimonyForm({Key? key, this.entry}) : super(key: key);

  @override
  _NewTestimonyFormState createState() => _NewTestimonyFormState(entry: entry);
}

class _NewTestimonyFormState extends State<NewTestimonyForm>
  ...
  _NewTestimonyFormState({required this.entry});
  ...
  Widget build(BuildContext context) {
    return BlocProvider<NewTestimoniesBloc>(
      create: (BuildContext context) =>
          getIt<TestimoniesBloc>()..add(NewTestimonyStarted()),
      child: BlocBuilder<NewTestimoniesBloc, TestimoniesState>(
        builder: (BuildContext context, state) {
          if (state is NewTestimonyState) {
                          ...
                          children: [
                            Container(
                              ...
                              child: TextFormField(
                                ...
                                autofocus: true,
                                onChanged: (value) {
                                  BlocProvider.of<NewTestimoniesBloc>(context)
                                    ..add(NewTestimonyRequestChanged(
                                        testimony: value.toString()));
                                },
                                  ...
                                  children: [
                                    AnonymousCheckbox(
                                      ...
                                      onChanged: (bool state) {
                                        BlocProvider.of<NewTestimoniesBloc>(
                                            context)
                                          ..add(NewTestimonyAnonymousChanged(
                                              isAnonymous: state));
                                      },
                                    ),
                                    SizedBox(width: 4),
